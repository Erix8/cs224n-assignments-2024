...
\begin{parts}
    ...
    \begin{adjustwidth}{}{}
    \begin{tabular}{ l | l | l }
    Stack & Transition & New dependency \\ \hline
    [ROOT] & Initial Configuration & \\
    $[$ROOT, I] & \texttt{SHIFT} & \\
    $[$ROOT, I, presented] & \texttt{SHIFT} & \\
    $[$ROOT, presented] & \texttt{LEFT-ARC} & presented$\to$I  \\
    $[$ROOT, presented, my] & \texttt{SHIFT}  \\
    $[$ROOT, presented, my, findings] & \texttt{SHIFT}  \\
    $[$ROOT, presented, findings] & \texttt{LEFT-ARC} & findings$\to$my  \\
    $[$ROOT, presented] & \texttt{RIGHT-ARC} & presented$\to$findings  \\
    $[$ROOT, presented, at] & \texttt{SHIFT} & \\
    $[$ROOT, presented, at, the] & \texttt{SHIFT} & \\
    $[$ROOT, presented, at, the, NLP] & \texttt{SHIFT} & \\
    $[$ROOT, presented, at, the, NLP, conference] & \texttt{SHIFT} & \\ 
    $[$ROOT, presented, at, the, conference] & \texttt{LEFT-ARC} & conference$\to$NLP  \\ 
    $[$ROOT, presented, at, conference] & \texttt{LEFT-ARC} & conference$\to$the  \\ 
    $[$ROOT, presented, conference] & \texttt{LEFT-ARC} & conference$\to$at  \\
    $[$ROOT, presented] & \texttt{RIGHT-ARC} & presented$\to$conference  \\
    $[$ROOT] & \texttt{RIGHT-ARC} & ROOT$\to$presented  \\ 
    \end{tabular}\newline
    \end{adjustwidth} 
    }
    ...
    \ifans{
    It takes $2n$ steps (the initial configuration is not a step). Each of the $n$ words is shifted onto the stack exactly once ($n$ \texttt{SHIFT} transitions), 
    and each of the $n$ non-ROOT words receives exactly one incoming dependency edge, so exactly $n$ arc transitions (\texttt{LEFT-ARC}/\texttt{RIGHT-ARC}) are applied; 
    parsing stops when the buffer is empty and only ROOT remains on the stack.
    }
    ...
    \begin{enumerate}[label=\roman*.]
        ...
        \ifans{
        Let $\bz = \bx\bW + \bb_1$, so $z_i = \sum_k x_k W_{ki} + b_{1,i}$ and $h_i = \relu(z_i)$. Then $\frac{\partial z_i}{\partial x_j} = W_{ji}$ and $\frac{\partial h_i}{\partial z_i} = \mathbf{1}\{z_i>0\}$, so by the chain rule
        \[
        \frac{\partial h_i}{\partial x_j} = W_{ji}\,\mathbf{1}\{z_i > 0\} = W_{ji}\,\mathbf{1}\{(\bx\bW + \bb_1)_i > 0\}.
        \]
        }
        ...
        \ifans{
        Since $\by$ is one-hot at the true label $c$, $CE(\by,\byt) = -\log\byt_c$. Using the hint, $\frac{\partial CE}{\partial\byt_j} = -\by_j/\byt_j$, which is $-1/\byt_c$ for $j=c$ and $0$ otherwise; and the softmax derivative is $\frac{\partial\byt_j}{\partial\bl_i} = \byt_i(\delta_{ij}-\byt_j)$, where $\delta_{ij}$ is the Kronecker delta ($\delta_{ij}=1$ if $i=j$, $0$ otherwise), which merges the two cases $i=j$ and $i\neq j$ into one expression. Hence
        \[
        \frac{\partial CE(\by,\byt)}{\partial\bl_i} = -\frac{1}{\byt_c}\frac{\partial\byt_c}{\partial\bl_i}
        = \begin{cases} \byt_i, & i\neq c \\ \byt_c - 1, & i = c \end{cases}
        = \byt_i - \by_i ,
        \]
        i.e.\ the gradient vector is $\byt - \by$.
        }
    ...
    \end{enumerate}
    \ifans{
    The trained parser achieved a \textbf{dev UAS of 88.65} (best over all epochs) and a \textbf{test UAS of 89.15}. (Model trained on an Apple M4 chip with 16\,GB memory.)
    }
...
\begin{enumerate}[label=\roman*.]
    \item \textbf{Error type}: Verb Phrase Attachment Error. \\
          \textbf{Incorrect dependency}: acquisition $\to$ citing. \\
          \textbf{Correct dependency}: blocked $\to$ citing. \\
          \emph{(The participial phrase ``citing concerns\ldots'' is an adverbial clause that should modify the main verb \textit{blocked}, not \textit{acquisition}.)}
    \item \textbf{Error type}: Modifier Attachment Error. \\
          \textbf{Incorrect dependency}: elements $\to$ most. \\
          \textbf{Correct dependency}: crucial $\to$ most. \\
          \emph{(The adverb \textit{most} should modify the adjective \textit{crucial}, not the noun \textit{elements}.)}
    \item \textbf{Error type}: Prepositional Phrase Attachment Error. \\
          \textbf{Incorrect dependency}: declined $\to$ decision. \\
          \textbf{Correct dependency}: reasons $\to$ decision. \\
          \emph{(The prepositional phrase ``for the government decision'' should modify \textit{reasons} (reasons for the decision), not \textit{declined}.)}
    \item \textbf{Error type}: Coordination Attachment Error. \\
          \textbf{Incorrect dependency}: affects $\to$ one. \\
          \textbf{Correct dependency}: plants $\to$ one. \\
          \emph{(In ``\ldots plants and one in Quebec'' the second conjunct \textit{one} should attach to the first conjunct \textit{plants}, not to the verb \textit{affects}.)}
\end{enumerate}
}
...
\part[2] Recall in part (e), the parser uses features which includes words and their part-of-speech (POS) tags. Explain the benefit of using part-of-speech tags as features in the parser?
    ...
    \ifans{
    POS tags encode coarse syntactic categories (noun, verb, adjective, preposition, \ldots) that are shared across many words, so they help the parser in two ways. First, they provide syntactic signal that a bare word string lacks --- e.g.\ knowing that a token is a preposition or an auxiliary verb strongly indicates how it should attach, and they let the parser distinguish words that are spelled the same but play different roles (such as \textit{bank} as a noun vs.\ a verb). Second, because a small set of tags covers the whole vocabulary, POS tags let the model \emph{generalize} to rare or completely unseen words that never appeared (or appeared too rarely) in training --- the parser can still reason about an unknown word via its POS tag, mitigating data sparsity.
    }
