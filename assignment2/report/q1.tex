...
\ifans{Since $\mathbf{y}$ is a one-hot vector with 1 at index $o$ and 0 elsewhere, all terms in the sum vanish except $w = o$, leaving $-1 \cdot \log(\hat{y}_o) = -\log(\hat{y}_o)$, which is exactly the naive-softmax loss.}
...
\ifans{
\begin{align*}
\bJ_{\text{naive-softmax}} &= -\log \hat{y}_o
= -\bu_o^\top \bv_c + \log\left(\sum_{k\in\text{Vocab}} \exp(\bu_k^\top \bv_c)\right),\\[2mm]
\frac{\partial \bJ_{\text{naive-softmax}}}{\partial \bv_c}
&= -\bu_o + \frac{\sum_{k\in\text{Vocab}} \exp(\bu_k^\top \bv_c)\,\bu_k}{\sum_{k\in\text{Vocab}} \exp(\bu_k^\top \bv_c)}
= -\bu_o + \sum_{k\in\text{Vocab}} \hat{y}_k \bu_k .
\end{align*}
Since $\by$ is one-hot with a $1$ at index $o$, we have $\bu_o = \bU\by$, and $\sum_k \hat{y}_k \bu_k = \bU\hat{\by}$. Therefore, in vectorized form (a column vector with the same shape as $\bv_c$):
\[
\boxed{\frac{\partial \bJ_{\text{naive-softmax}}(\bv_c, o, \bU)}{\partial \bv_c} = \bU(\hat{\by} - \by)}
\]

The gradient vanishes iff $\bU(\hat{\by}-\by)=\bzero$:
\begin{itemize}
\item If the columns of $\bU$ are linearly independent, then $\bU$ is injective, so this holds \emph{iff} $\hat{\by}=\by$, i.e.\ the model predicts probability $1$ for the true outside word $o$ and $0$ for every other word. 
\item If the columns of $\bU$ are linearly dependent, the gradient can also vanish whenever $(\hat{\by}-\by)$ lies in the null space of $\bU$, even when the prediction is not perfect.
\end{itemize}

The gradient equals $\bU\hat{\by}-\bU\by$, so gradient descent performs
$\bv_c \leftarrow \bv_c - \alpha \bU\hat{\by} + \alpha \bU\by$:
\begin{itemize}
    \item $+\alpha\bU\by = \alpha\bu_o$ \textbf{pulls} $\bv_c$ \emph{toward} the true outside vector $\bu_o$, increasing the score $\bu_o^\top\bv_c$ so the correct word $o$ becomes more probable.
    \item $-\alpha\bU\hat{\by}$ \textbf{pushes} $\bv_c$ \emph{away from} every word vector $\bu_k$ weighted by its predicted probability $\hat{y}_k$, most strongly away from words that are currently over-predicted, lowering their scores.
\end{itemize}
Together these update $\bv_c$ so the model concentrates probability mass on the true word $o$ and reduces mass on incorrect words, driving $\hat{\by}$ toward the one-hot $\by$.
}
...
\ifans{
L2 normalization discards each word vector's \emph{magnitude}, keeping only its \emph{direction}. For $\bu_x = \alpha\bu_y$ ($\alpha>0$), both normalize to the same unit vector, so the scale $\alpha$ is lost.

Since the task decides the sign from the \emph{sum} of embeddings, a word's magnitude encodes how strongly it pushes the sum (hence the sign). So normalization \textbf{takes away} useful information when magnitudes differ and matter --- e.g.\ a strong word with $\alpha>1$ should weigh more than a weak same-direction word, but after normalization they weigh equally, possibly flipping the sum's sign.

It would \textbf{not} hurt when magnitude carries no signal for the decision --- e.g.\ if all vectors already share the same norm $c$, then $\bu_i/\|\bu_i\|=\bu_i/c$ and the sum is scaled by $1/c>0$, leaving its sign unchanged. That is, normalization is harmless whenever the decision depends only on directions, not lengths.
}
% Question 1-D
\part[5] 
...
\ifans{
Since $\bJ = -\log \hat{y}_o = -\bu_o^\top \bv_c + \log\left(\sum_{k\in\text{Vocab}}\exp(\bu_k^\top\bv_c)\right)$, differentiating with respect to a single outside vector $\bu_w$ gives
\[
\frac{\partial \bJ}{\partial \bu_w}
= -\mathbf{1}\{w=o\}\,\bv_c + \frac{\exp(\bu_w^\top\bv_c)}{\sum_{k}\exp(\bu_k^\top\bv_c)}\,\bv_c
= (\hat{y}_w - y_w)\bv_c ,
\]
since $y_w = \mathbf{1}\{w=o\}$. Concretely:
\begin{itemize}
    \item \textbf{Case $w=o$:} $y_o=1$, so $\dfrac{\partial \bJ}{\partial \bu_o} = (\hat{y}_o - 1)\bv_c$.
    \item \textbf{Case $w\neq o$:} $y_w=0$, so $\dfrac{\partial \bJ}{\partial \bu_w} = \hat{y}_w\bv_c$.
\end{itemize}
In both cases the answer is the vector $(\hat{y}_w-y_w)\bv_c$, written in terms of $\hat{\by}$, $\by$, and $\bv_c$ (with $\hat{y}_w, y_w$ scalars), and each derivative has the same shape as $\bu_w$.
}
% Question 1-E
\part[1]
...
\ifans{
From Question 1-D, each column is $\frac{\partial \bJ}{\partial \bu_w} = (\hat{y}_w - y_w)\bv_c$. Hence
\[
\frac{\partial \bJ(\bv_c, o, \bU)}{\partial \bU}
=
\bv_c(\hat{\by}-\by)^\top,
\]
a matrix whose $w$-th column is $(\hat{y}_w-y_w)\bv_c$, for $w=1,\dots,|\text{Vocab}|$. Its shape is $d\times|\text{Vocab}|$, the same as $\bU$.
}
\end{parts}