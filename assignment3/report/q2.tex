...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{
        A single reference is problematic because BLEU only credits the $n$-grams a candidate literally shares with the given reference, so a correct-but-differently-worded translation is under-scored merely for not matching that one reference. With multiple references, BLEU takes the \emph{maximum} count of each $n$-gram across references, so a candidate matching any valid phrasing receives credit --- a fairer estimate of quality.
        }
    ...
    \begin{subparts}
        ...
        \ifans{
        Yes --- translation quality clearly improves as training proceeds. For the example source sentence \emph{``我还澄清了在该会议上提出的若干事项。''} (reference: \emph{``i was able to provide clarification on some of the matters which were raised at that meeting.''}), the first (highest-scoring) beam hypothesis at each iteration is:

        \begin{itemize}
            \item \textbf{Iteration 200} (score $\approx -33.8$): \emph{``it is the united nations of the united nations of the united nations of the united nations.''} --- the model has collapsed into a degenerate repetition loop and the output has nothing to do with the source.
            \item \textbf{Iteration 3000} (score $\approx -15.3$): \emph{``i would also like to clarify the number of questions in the conference.''} --- now fluent, on-topic English (it picks up \emph{clarify} and \emph{in the conference}), but the content is still wrong (``number of questions'' for the matters raised) and it drifts from the reference.
            \item \textbf{Last iteration (19000)} (score $\approx -6.7$): \emph{``i have also clarified a number of matters raised during the meeting.''} --- almost identical to the gold translation, with only minor wording/tense differences.
        \end{itemize}

        So the model progresses from incoherent repetition, to on-topic-but-content-wrong, to an essentially correct translation.
        }
        ...
        \ifans{
        The hypotheses are near-synonymous paraphrases of one another, ranked by model score. At the last iteration the top hypothesis is \emph{``i have also clarified a number of matters raised during the meeting.''} (score $\approx -6.7$), and the remaining candidates differ by only a single word --- swapping a content word, a preposition, or the location noun while staying grammatical and on-topic:

        \begin{itemize}
            \item \emph{``i have also clarified a number of \textbf{issues} raised during the meeting.''} (score $\approx -6.8$) --- \emph{matters} $\rightarrow$ \emph{issues};
            \item \emph{``i have also clarified a number of matters raised \textbf{at} the meeting.''} (score $\approx -7.6$) --- \emph{during} $\rightarrow$ \emph{at};
            \item \emph{``i have also clarified a number of matters raised during the \textbf{conference}.''} (score $\approx -8.0$) --- \emph{meeting} $\rightarrow$ \emph{conference} (and further down the list, \emph{session}).
        \end{itemize}

        All ten candidates therefore share the same syntactic frame and meaning but swap individual tokens; the highest-scoring one is the most natural and nearly equals the reference, while the lower-ranked variants stay fluent and plausible but are scored slightly lower.
        }
