...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \ifans{
        In general, translation quality should improve over training: as the training loss decreases, the model learns better source--target alignments and more fluent output, so the first (highest-scoring) beam hypothesis at later iterations should be progressively more accurate and natural. I could not observe the actual hypotheses, since producing the \texttt{outputs/beam\_search\_diagnostics/} data requires training the model on a GPU, which I do not have; the above is therefore an inference rather than a report of observed translations.
        }
        ...
        \ifans{
        Beam search retains the top-$k$ hypotheses by model score, so the 10 hypotheses should be ranked, near-synonymous variants of one another: they should share the same overall meaning and structure but can differ in word choice, function words, and word order. The first (highest-scoring) hypothesis should be the most likely and fluent, while lower-ranked ones should be slightly less likely but still plausible. Again this is an inference; the observed hypotheses require a trained model, which I cannot run without a GPU.
        }
