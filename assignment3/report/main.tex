...
\usepackage{fontspec}
% \cherokeefam is unused anywhere in this document, so we do not load it.
% The font "FreeSerif" is also not installed by default on macOS/TeX Live,
% which caused fontspec errors. Re-enable only if you actually add Cherokee text.
% \newfontfamily{\cherokeefam}{FreeSerif}

% "Latin Modern Mono" resolves by name on Linux but NOT on macOS (CoreText doesn't
% index TeX's fonts). Loading by filename works on any TeX Live install because
% the font ships with the distribution.
\newfontfamily{\monofam}{lmmono10-regular.otf}
% Make \texttt/\ttfamily (used by \texttt{...} throughout) render with the same
% LM Mono font. The default mono font under fontspec/CoreText on macOS would
% otherwise fall back and lose its monospaced code look.
...
\lstset{language=bash,
        ...
        keywordstyle=\monofam, % no bold mono (lmmono has no bold face)
        commentstyle=\monofam, % no italic mono
}
...
% \ifans{} renders a solution in red when \answerstrue is set (mirrors Assignment 2's format).
% Leave the \ifans line below commented out to hide all solutions.
