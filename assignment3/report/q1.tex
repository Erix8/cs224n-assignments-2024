...
\end{lstlisting}
    ...
    \ifans{
    The full model (embedding size 1024, hidden size 768) achieved a \textbf{corpus BLEU score of 19.93 on the test set} (21.27 on the dev set), exceeding the required threshold of 18. It was trained on the complete \texttt{zh\_en\_data} corpus on a single NVIDIA RTX 4090 (AutoDL) and evaluated with \texttt{sh run.sh test} (sacreBLEU). The learning-rate schedule triggered early stopping after five trials, by which point the model had converged.
    }
