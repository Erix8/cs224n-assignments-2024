...
\begin{parts}
    ...
    \begin{subparts}
        ...
        \begin{align*}
        \mathbf{H}_{\text{perm}}
        &= \text{softmax}\!\left(\frac{\mathbf{P}\mathbf{Q}\mathbf{K}^{\top}\mathbf{P}^{\top}}{\sqrt{d}}\right)\mathbf{P}\mathbf{V}
         = \mathbf{P}\,\text{softmax}\!\left(\frac{\mathbf{Q}\mathbf{K}^{\top}}{\sqrt{d}}\right)\mathbf{P}^{\top}\mathbf{P}\mathbf{V}
         = \mathbf{P}\,\text{softmax}\!\left(\frac{\mathbf{Q}\mathbf{K}^{\top}}{\sqrt{d}}\right)\mathbf{V}
         = \mathbf{P}\mathbf{H}.
        \end{align*}
        ...
        \begin{align*}
        \mathbf{Z}_{\text{perm}}
        &= \text{ReLU}\!\left(\mathbf{P}\mathbf{H}\mathbf{W}_1 + \mathbf{1}\mathbf{b}_1\right)\mathbf{W}_2 + \mathbf{1}\mathbf{b}_2
         = \text{ReLU}\!\left(\mathbf{P}\left(\mathbf{H}\mathbf{W}_1 + \mathbf{1}\mathbf{b}_1\right)\right)\mathbf{W}_2 + \mathbf{P}\mathbf{1}\mathbf{b}_2 \\
        &= \mathbf{P}\,\text{ReLU}\!\left(\mathbf{H}\mathbf{W}_1 + \mathbf{1}\mathbf{b}_1\right)\mathbf{W}_2 + \mathbf{P}\mathbf{1}\mathbf{b}_2
         = \mathbf{P}\left[\text{ReLU}\!\left(\mathbf{H}\mathbf{W}_1 + \mathbf{1}\mathbf{b}_1\right)\mathbf{W}_2 + \mathbf{1}\mathbf{b}_2\right]
         = \mathbf{P}\mathbf{Z}.
        \end{align*}
    ...
    \begin{subparts}
        ...
        \ifans{Yes. After adding the position embeddings, the input becomes $\mathbf{X}_{\text{pos}} = \mathbf{X} + \mathbf{\Phi}$, where $\mathbf{\Phi}$ encodes the absolute position of each token and is \emph{not} permuted along with the tokens. Hence permuting the tokens now gives $\mathbf{P}\mathbf{X} + \mathbf{\Phi} \neq \mathbf{P}(\mathbf{X} + \mathbf{\Phi})$ in general, so the output is no longer simply the permuted output of the original input: the position-dependent signal breaks the permutation equivariance, and the model can now distinguish different orderings of the same tokens.}
        ...
        \ifans{Yes. The embeddings are built from sine and cosine, which are periodic functions of the position $t$, so two different positions can receive the same embedding. For example, in the simplest case $d = 2$, $\mathbf{\Phi}(t) = \left(\sin t,\ \cos t\right)$ is periodic with period $2\pi$, so positions $t$ and $t + 2\pi k$ ($k \in \mathbb{Z}$) yield identical embeddings. More generally, the $i$-th frequency pair $\left(\sin\left(t/10000^{2i/d}\right), \cos\left(t/10000^{2i/d}\right)\right)$ repeats every $2\pi\cdot 10000^{2i/d}$ positions, so if the sequence is long enough for two positions to differ by a common period of all the sinusoids, their full position embeddings coincide (and even before an exact collision, far-apart positions can receive very similar --- and, with floating-point precision, eventually numerically equal --- embeddings).}
    \end{subparts}
\end{parts}
